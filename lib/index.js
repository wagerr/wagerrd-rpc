...
async function rpc (request) {
  ...
  const {status, data} = await axios.post(`http://${self.host}:${self.port}`, request, requestConfig)
  if (status === 401) {
  ...
  if (data.error)
    throw new Error(`Wagerr JSON-RPC: ${data.error.code}: ${data.error.message}`)
  return data
}
...
// == Wallet ==
  ...
  multiSend: 'str',
  placeBet: 'str str float str str', //bet
